...
public class PlayNowCustom1 extends AppCompatActivity{
    int c = 0, timeOver = 0, tt = 0, lost = 0, resEt = 0, bombNo = 72, timerValue = 120000;
    ...
    public void cell(View view) {
            ...
            if(resEt == 0) {
                ...
                t = new CountDownTimer(timerValue, 1000) {
                    ...
                    public void onFinish() {
                        ...
                        timeOver++;
                        ringer.stop();
                        timerGone = MediaPlayer.create(PlayNowCustom1.this, R.raw.timergone);
                        timerGone.start();
                        tt++;
                    }
                }.start();
        ...
        if(lost == 0) {
            TextView status = findViewById(R.id.status);
            if (opened == (81 - bombNo)) {//Change for new number of bombs
                if (timeOver != 0) {
                    timeOver--;
                    ...
                    status.setText("Win. Score " + tim * 600000 / timerValue * bombNo / 50 * 2);
                    TextView status1 = findViewById(R.id.statusBomb);
                    status1.setText("Bombs: " + bombNo);
                    if(whetherEight == 0) {
                        dataBaseHandler db = new dataBaseHandler(PlayNowCustom1.this);
                        Date d = new Date(System.currentTimeMillis());
                        highScore hs = new highScore(tim * 600000 / timerValue * bombNo / 50 * 2, fmt.format(d));
                        db.addHighScoreEight(hs);
                        db.close();
                    } else {
                        dataBaseHandler db = new dataBaseHandler(PlayNowCustom1.this);
                        Date d = new Date(System.currentTimeMillis());
                ...
                fireworks = MediaPlayer.create(PlayNowCustom1.this, R.raw.fireworks);
                ringer.stop();
                fireworks.start();
        ...
        for(int i = 0; i < bombNo; i++) {//Change for new number of bombs
            ...
            if(opened == (81 - bombNo)) {//Change for new number of bombs
                ...
                new CountDownTimer(1000, 1000) { // 5000 = 5 sec
                    ...
                    public void onFinish() {
                        if(finalI == 0) {
                            ring = MediaPlayer.create(PlayNowCustom1.this, R.raw.ring);
                            ring.start();
                        }
    ...
    private void openSurroundings(int i, int j) {
        if(whetherEight == 0) {
            ...
            if (arrData[i][j] == 0) {
                ...
                if (i + 1 < 9 && arrData[i + 1][j] != 111 && arr[i + 1][j] != 'A' && arr[i + 1][j] != 'M' && arr[i + 1][j] != 'O') {
                    openSurroundings(i + 1, j);
                    arr[i + 1][j] = 'O';
                }
                if (j + 1 < 9 && arrData[i][j + 1] != 111 && arr[i][j + 1] != 'A' && arr[i][j + 1] != 'M' && arr[i][j + 1] != 'O') {
                    openSurroundings(i, j + 1);
                    arr[i][j + 1] = 'O';
                }
                ...
                if (i - 1 >= 0 && j + 1 < 9 && arrData[i - 1][j + 1] != 111 && arr[i - 1][j + 1] != 'A' && arr[i - 1][j + 1] != 'M' && arr[i - 1][j + 1] != 'O') {
                    openSurroundings(i - 1, j + 1);
                    arr[i - 1][j + 1] = 'O';
                }
                if (j - 1 >= 0 && i + 1 < 9 && arrData[i + 1][j - 1] != 111 && arr[i + 1][j - 1] != 'A' && arr[i + 1][j - 1] != 'M' && arr[i + 1][j - 1] != 'O') {
                    openSurroundings(i + 1, j - 1);
                    arr[i + 1][j - 1] = 'O';
                }
                if (i + 1 < 9 && j + 1 < 9 && arrData[i + 1][j + 1] != 111 && arr[i + 1][j + 1] != 'A' && arr[i + 1][j + 1] != 'M' && arr[i + 1][j + 1] != 'O') {
                    openSurroundings(i + 1, j + 1);
                    arr[i + 1][j + 1] = 'O';
                }
            }
    ...
    public void reset(View view) {
        ...
        ringer = MediaPlayer.create(PlayNowCustom1.this,R.raw.ringer);
        ringer.start();
        ringer.setLooping(true);
    }
    ...
    public void onBackPressed() {
        ...
        PerfectLoopMediaPlayer.create(PlayNowCustom1.this, R.raw.bg);
    }
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        DataBaseHandlerMT dataBaseHandlerMT = new DataBaseHandlerMT(PlayNowCustom1.this);
        int n;
        try {
